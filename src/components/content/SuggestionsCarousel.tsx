...
function CarouselCard({ game }: { game: SuggestedGame }) {
...
export default function SuggestionsCarousel({ games }: { games: SuggestedGame[] }) {
  const safeGames = Array.isArray(games) ? games : [];
  const gamesLength = safeGames.length;
  ...
  const extendedSuggestions = [...safeGames, ...safeGames, ...safeGames];
  ...
  const nextSlide = useCallback(() => {
    if (currentIndex >= extendedSuggestions.length - 1) return;
    setIsTransitioning(true);
    setCurrentIndex(prev => prev + 1);
  }, [currentIndex, extendedSuggestions.length]);
  ...
  return (
    ...
    >
      ...
      <div className="suggestions-carousel-window">
        ...
        >
          {extendedSuggestions.map((game, index) => (
